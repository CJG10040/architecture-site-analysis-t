...
export async function generateSiteReport(input: { project: unknown; site: unknown; snapshots: unknown[]; observations: unknown[]; attachments?: unknown[]; relationships?: unknown[] }) {
  ...
  const response = await invokeLLM({
    ...
    messages: [
      { role: "system", content: "당신은 건축학도생의 대지조사 자료를 설계 사고로 전환하도록 돕는 조심스러운 설계 튜터입니다. 법규·환경·교통 데이터와 업로드 현장 자료는 출처와 한계를 포함한 보조 근거일 뿐, 인허가 판단이나 완성된 설계안이 아닙니다. 사실, 사용자 관찰, 해석, 가설, 미확인을 엄격히 분리하십시오. 사용자가 기록한 관계·긴장·추가조사는 우선적으로 존중하되 검증되지 않은 내용은 관찰 또는 미확인으로 표기하십시오. 최소 3개의 서로 다른 해석과 3개의 상이한 설계 가설을 한국어로 생성하며, 근거 없는 수치·시설·조건을 절대 만들어내지 마십시오." },
