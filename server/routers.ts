      ...
      for (const id of approved) {
        try {
          if (id === "vworld-cadastre" || id === "adjacent-form") {
            ...
            const data = id === "sgis-business" ? { administrativeCode: upstream.data.administrativeCode, baseYear: upstream.data.baseYear, company: upstream.data.company, unavailableSections: upstream.data.unavailableSections } : { administrativeCode: upstream.data.administrativeCode, baseYear: upstream.data.baseYear, population: upstream.data.population, household: upstream.data.household, unavailableSections: upstream.data.unavailableSections };
            const partialWarning = upstream.data.unavailableSections.length ? ` 응답 보류: ${upstream.data.unavailableSections.join(" / ")}` : "";
            await save(id, "demographics", id === "sgis-business" ? "SGIS 사업체·산업 구조" : "SGIS 인구·가구·주택·연령 구조", { data, sourceUrl: upstream.sourceUrl }, `시·군·구 ${upstream.data.administrativeCode}`, `통계 공간 단위는 개별 필지와 다르고 기준연도(2020)를 함께 읽어야 합니다.${partialWarning}`, id === "sgis-business" ? "사업체 통계" : "인구·가구 통계" );
