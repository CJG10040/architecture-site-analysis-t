...
export const appRouter = router({
  ...
  projects: router({
    ...
    resetResearchData: protectedProcedure.input(z.object({ projectId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { await ensureProjectAccess(input.projectId, ctx.user.id, ctx.user.role === "admin"); await db.resetProjectResearchData(input.projectId); return { success: true as const }; }),
