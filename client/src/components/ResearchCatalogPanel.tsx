...
export function ResearchCatalogPanel({ project, onPlanChange }: { project: LocalProject; onPlanChange: (plan: ResearchPlan) => void }) {
  ...
  const partialCount = researchCatalog.filter(item => item.status === "partial").length;
  const plannedCount = researchCatalog.filter(item => item.status === "planned").length;
  const fieldCount = researchCatalog.filter(item => item.status === "field").length;
  ...
  return <section className="mt-5 border border-stone-300 bg-[#fbfaf7] p-4">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex gap-2 text-xs"><Badge className="border-0 bg-[#e8f0e4] text-[#42633d]">사용 가능 {implementedCount}</Badge><Badge className="border-0 bg-[#fff5dd] text-[#8a621c]">부분 구현 {partialCount}</Badge><Badge className="border-0 bg-[#fff0eb] text-[#8b3a2d]">보충 필요 {plannedCount}</Badge><Badge className="border-0 bg-[#edf1f7] text-[#4a617f]">현장 {fieldCount}</Badge></div>
    </div>
    ...
    <div className="mt-4 border border-[#bfcdb5] bg-[#f3f8f0] p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-stone-900">기본 조사 테마 · 거시에서 미시로 좁히기</p><p className="mt-1 text-xs leading-5 text-stone-600">테마를 켜두면 아래의 기본 자료가 자동 선택됩니다. 부분 구현 자료는 자동 수집과 상세 원본 보완을 함께 사용하고, 보충 구현 필요·현장 자료는 상세 기록이나 원본 파일로 조사합니다.</p></div><Badge className="border-0 bg-white text-[#42633d]">선택 자료 {plan.selectedCatalogIds.length}개</Badge></div><div className="mt-3 grid gap-3 xl:grid-cols-2">{researchThemes.map(theme => <div key={theme.id} className="border border-[#cbd9c4] bg-white p-3"><label className="flex items-start gap-2"><input type="checkbox" checked={plan.selectedThemeIds.includes(theme.id)} onChange={() => toggleTheme(theme.id)} className="mt-1" /><span><strong className="text-sm text-stone-900">{theme.title}</strong><span className="mt-1 block text-xs leading-5 text-stone-600">{theme.description}</span></span></label><div className="mt-3 space-y-1.5 border-t border-stone-100 pt-2">{theme.catalogIds.map(catalogId => { const item = researchCatalog.find(entry => entry.id === catalogId); if (!item) return null; const collected = project.researchNotes.some(note => note.catalogId === item.id); return <label key={item.id} className="flex items-start gap-2 text-xs"><input type="checkbox" checked={plan.selectedCatalogIds.includes(item.id)} onChange={() => toggleCatalog(item.id)} className="mt-0.5" /><span className="min-w-0 flex-1"><span className="text-stone-700">{item.title}</span><span className="ml-1 text-[10px] text-stone-400">{statusLabels[item.status]}</span></span><span className={`text-[10px] ${collected ? "text-[#42633d]" : "text-stone-400"}`}>{collected ? "조사됨" : "미조사"}</span></label>; })}</div></div>)}</div></div>

    <div className="mt-4 border border-[#bfcdb5] bg-[#f3f8f0] p-3"><div className="flex items-center gap-2"><MapPinned className="h-4 w-4 text-[#46603e]" /><p className="text-sm font-medium text-stone-900">결정론적 대지 근거 · AI 전에 계산</p></div><p className="mt-1 text-xs leading-5 text-stone-600">API별 카드 요약을 그대로 AI에 보내지 않고, geometry·속성·관계를 먼저 로컬에서 계산합니다. 아래 결과가 부족하면 AI가 결론을 만들지 않고 데이터 공백으로 표시해야 합니다.</p><div className="mt-3 grid gap-3 md:grid-cols-2"><div className="border border-[#cbd9c4] bg-white p-3"><p className="text-[10px] font-mono tracking-[.14em] text-[#46603e]">FACT</p>{evidence.facts.map(fact => <p key={fact} className="mt-1 text-xs leading-5 text-stone-700">· {fact}</p>)}</div><div className="border border-[#cbd9c4] bg-white p-3"><p className="text-[10px] font-mono tracking-[.14em] text-[#46603e]">RELATION</p>{evidence.relations.map(relation => <p key={relation} className="mt-1 text-xs leading-5 text-stone-700">· {relation}</p>)}</div><div className="border border-[#e8d5ae] bg-[#fffaf0] p-3"><p className="text-[10px] font-mono tracking-[.14em] text-[#8a621c]">UNKNOWN / DATA GAP</p>{evidence.unknowns.map(unknown => <p key={unknown} className="mt-1 text-xs leading-5 text-stone-700">· {unknown}</p>)}</div><div className="border border-[#cbd9c4] bg-white p-3"><p className="text-[10px] font-mono tracking-[.14em] text-[#46603e]">NEXT FIELD CHECK</p>{evidence.next.map(next => <p key={next} className="mt-1 text-xs leading-5 text-stone-700">· {next}</p>)}</div></div></div>

    {spatialSummary.length > 0 && <div className="mt-4 border border-[#bfcdb5] bg-[#f3f8f0] p-3"><div className="flex items-center gap-2"><MapPinned className="h-4 w-4 text-[#46603e]" /><p className="text-sm font-medium text-stone-900">현재 프로젝트의 공간 레이어</p></div><div className="mt-2 grid gap-2 md:grid-cols-2">{spatialSummary.map(layer => <div key={layer.id} className="border border-[#cbd9c4] bg-white p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-medium text-stone-900">{layer.title}</p><span className="text-[10px] text-[#42633d]">지도 표시 중</span></div><p className="mt-1 text-[11px] leading-5 text-stone-600">보존 {layer.metrics.featureCount.toLocaleString("ko-KR")}개 / 응답 {layer.totalFeatureCount.toLocaleString("ko-KR")}개 · geometry {layer.metrics.geometryTypes}</p><p className="mt-1 text-[11px] text-stone-500">속성 필드 {layer.metrics.propertyCount}개 · {layer.truncated ? "300개까지 보존, 원 응답은 더 많음" : "응답 객체를 모두 보존"}</p>{layer.metrics.totalLengthMeters > 0 && <p className="mt-1 text-[11px] text-stone-500">보존 선형 길이 약 {Math.round(layer.metrics.totalLengthMeters).toLocaleString("ko-KR")}m · 교통량 아님</p>}{layer.metrics.totalAreaSqm > 0 && <p className="mt-1 text-[11px] text-stone-500">보존 footprint 면적 약 {Math.round(layer.metrics.totalAreaSqm).toLocaleString("ko-KR")}㎡ · 건폐율 아님</p>}{layer.id === "vworldRoads" && (() => { const road = analyzeRoadLayer(layer.features); const relation = roadBoundaryRelation(layer.features, project.site.boundary); const categories = Object.entries(road.categoryCounts).sort((a, b) => b[1] - a[1]).slice(0, 4).map(([name, count]) => `${name} ${count}개`).join(" · "); return <div className="mt-2 border-t border-stone-100 pt-2 text-[11px] leading-5 text-stone-500"><p><strong className="text-stone-600">도로 위계·유형</strong> {categories || "미확인"}</p><p><strong className="text-stone-600">폭</strong> {road.widthAttributeCount ? `실제 폭 속성 ${road.widthAttributeCount}개` : "실제 폭 속성 없음"} · {road.widthEstimatedCount ? `차로수×3.25m 추정 ${road.widthEstimatedCount}개` : "차로수 기반 추정 없음"} · {road.widthUnknownCount ? `폭 미확인 ${road.widthUnknownCount}개` : "폭 확인됨"}</p><p><strong className="text-stone-600">교통량</strong> {road.trafficAttributeCount ? `속성 확인 ${road.trafficAttributeCount}개` : "교통량 속성 없음 · 현장조사 필요"}</p><p><strong className="text-stone-600">대지 접근 참고</strong> {relation.nearestDistanceMeters === null ? "경계 또는 선형 geometry 미확인" : `가장 가까운 중심선 약 ${Math.round(relation.nearestDistanceMeters)}m · 30m 이내 ${relation.within30m}개 · 60m 이내 ${relation.within60m}개`}</p><p className="text-[10px] text-stone-400">{relation.note}</p></div>; })()}</div>)}</div><p className="mt-2 text-[11px] leading-5 text-stone-500">레이어가 지도에 보인다는 것은 공간객체의 위치를 확인했다는 의미입니다. 도로 접근성·건물 밀도·용도 관계 같은 설계 해석은 다음 분석 단계에서 별도로 계산합니다.</p></div>}
