...
import { createLocalProject, getActiveProject, normalizeWorkspace, projectSnapshot, type LocalProject, type ResearchNote, updateProject } from "@/static/model";
...
import { researchLayerGeoJson, researchNoteBundle, researchNoteRaw, researchNotesCsv, safeDownloadName } from "@/static/export";

const lenses = ["보행·접근", "지형·레벨", "일조·차폐", "지역 생활", "녹지·생태", "프로그램·상권"];
const newId = () => globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

function downloadText(name: string, text: string, mime = "application/json;charset=utf-8") {
...
export default function Home() {
  ...
  const exportProject = () => downloadText(`${safeDownloadName(project.title)}.json`, projectSnapshot(project));
  const exportResearchBundle = (note: ResearchNote) => downloadText(`${safeDownloadName(note.title, "research-note")}-상세.json`, researchNoteBundle(note));
  const exportResearchRaw = (note: ResearchNote) => { const raw = researchNoteRaw(note); const isJson = /^\s*[\[{]/.test(raw); downloadText(`${safeDownloadName(note.title, "research-note")}-원자료.${isJson ? "json" : "txt"}`, raw, isJson ? "application/json;charset=utf-8" : "text/plain;charset=utf-8"); };
  const exportResearchLayer = (note: ResearchNote) => { const geoJson = researchLayerGeoJson(note); if (geoJson) downloadText(`${safeDownloadName(note.title, "research-layer")}.geojson`, geoJson, "application/geo+json;charset=utf-8"); };
  const exportResearchCsv = () => downloadText(`${safeDownloadName(project.title)}-조사근거.csv`, researchNotesCsv(project.researchNotes), "text/csv;charset=utf-8");
...
function RecordPanel({ title, icon, draft, setDraft, onAdd, onRemove, onDownload, onDownloadRaw, onDownloadLayer, onExportAll, items, type }: { title: string; icon: React.ReactNode; draft: any; setDraft: (value: any) => void; onAdd: () => void; onRemove: (id: string) => void; onDownload?: (item: ResearchNote) => void; onDownloadRaw?: (item: ResearchNote) => void; onDownloadLayer?: (item: ResearchNote) => void; onExportAll?: () => void; items: any[]; type: "research" | "observation" }) { return <section className="border border-stone-300 bg-white p-4"><div className="flex items-center justify-between gap-3 text-stone-900"><div className="flex items-center gap-2">{icon}<h2 className="font-serif text-xl">{title}</h2></div>{type === "research" && onExportAll && items.length > 0 && <Button size="sm" variant="outline" onClick={onExportAll} className="border-stone-300"><Download className="mr-1.5 h-3.5 w-3.5" />전체 CSV</Button>}</div>{type === "research" ? <><Input value={draft.source} onChange={event => setDraft({ ...draft, source: event.target.value })} placeholder="출처 또는 제공기관" className="mt-3 border-stone-300" /><Input value={draft.title} onChange={event => setDraft({ ...draft, title: event.target.value })} placeholder="자료 제목" className="mt-2 border-stone-300" /><Textarea value={draft.summary} onChange={event => setDraft({ ...draft, summary: event.target.value })} placeholder="수집한 사실·수치·한계·설계에 읽을 점" className="mt-2 min-h-24 border-stone-300" /></> : <><Input value={draft.category} onChange={event => setDraft({ ...draft, category: event.target.value })} placeholder="관찰 유형" className="mt-3 border-stone-300" /><Input value={draft.title} onChange={event => setDraft({ ...draft, title: event.target.value })} placeholder="관찰 제목" className="mt-2 border-stone-300" /><Textarea value={draft.note} onChange={event => setDraft({ ...draft, note: event.target.value })} placeholder="현장에서 검증한 접근·소리·빛·보행·레벨차" className="mt-2 min-h-24 border-stone-300" /></>}<Button variant="outline" size="sm" onClick={onAdd} className="mt-2 border-stone-300"><Plus className="mr-1.5 h-4 w-4" />기록 추가</Button><div className="mt-4 space-y-2">{items.length === 0 ? <p className="text-xs text-stone-500">아직 기록이 없습니다.</p> : items.map(item => <article key={item.id} className="group border-l-2 border-[#a9684f] bg-[#fbfaf7] p-3"><div className="flex items-start justify-between gap-2"><div><p className="text-[11px] text-stone-500">{item.source ?? item.category}</p><p className="font-medium text-stone-900">{item.title}</p></div><div className="flex items-center gap-1"><>{type === "research" && onDownload && <button onClick={() => onDownload(item)} className="border border-stone-300 px-1.5 py-1 text-[10px] text-stone-600 hover:bg-stone-100" title="요약·상세·원자료·공간 레이어를 하나의 JSON으로 다운로드">상세 JSON</button>}{type === "research" && onDownloadRaw && (item.rawData || item.detail) && <button onClick={() => onDownloadRaw(item)} className="border border-stone-300 px-1.5 py-1 text-[10px] text-stone-600 hover:bg-stone-100" title="원자료 또는 상세 기록만 다운로드">원자료</button>}{type === "research" && onDownloadLayer && item.spatialLayer && <button onClick={() => onDownloadLayer(item)} className="border border-stone-300 px-1.5 py-1 text-[10px] text-[#46603e] hover:bg-[#f3f8f0]" title="추출된 공간자료를 GeoJSON으로 다운로드">GeoJSON</button>}</><button onClick={() => onRemove(item.id)} className="opacity-60 hover:text-[#8b4a38] group-hover:opacity-100" aria-label="기록 삭제"><Trash2 className="h-3.5 w-3.5" /></button></div></div><p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-stone-600">{item.summary ?? item.note}</p>{(item.detail || item.rawData) && <details className="mt-2 border-t border-stone-200 pt-2"><summary className="cursor-pointer text-[11px] font-medium text-[#46603e]">상세 원자료·속성 보기</summary><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words bg-[#f8f5ef] p-2 text-[10px] leading-4 text-stone-600">{item.detail ?? item.rawData}{item.rawDataTruncated ? "\n\n[저장 한도로 일부가 잘렸습니다]" : ""}</pre></details>}</article>)}</div></section>; }
