...
export default function Home() {
  ...
  const updateSite = (change: Partial<LocalProject["site"]>) => {
    ...
    commit({ ...project, site: { ...project.site, ...change, ...(positionChanged ? { parcels: undefined, pnu: undefined, parcelLabel: undefined } : {}) } });
  };
  const updateBoundary = (points: LocalProject["site"]["boundary"]) => {
    const metrics = boundaryMetrics(points);
    updateSite({ boundary: points, geoJson: boundaryGeoJson(points) ?? undefined, areaSqm: metrics.areaSqm || undefined, perimeterMeters: metrics.perimeterMeters || undefined, parcels: undefined, pnu: undefined, parcelLabel: undefined });
