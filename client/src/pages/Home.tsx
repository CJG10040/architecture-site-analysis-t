...
export default function Home() {
  ...
  const addCollectedResearch = (notes: LocalProject["researchNotes"]) => commit({ ...project, researchNotes: [...notes, ...project.researchNotes] });
  ...
  <SiteMapPicker apiKey={services.googleMapsKey ?? ""} latitude={project.site.latitude} longitude={project.site.longitude} address={project.site.address} boundary={project.site.boundary} onSiteChange={updateSite} onBoundaryChange={updateBoundary} onOpenSettings={openSettings} />
  <div className="mt-3 grid gap-3 sm:grid-cols-3"><Metric label="중심 좌표" value={`${project.site.latitude.toFixed(6)}, ${project.site.longitude.toFixed(6)}`} /><Metric label="경계 면적" value={project.site.areaSqm ? `${Math.round(project.site.areaSqm).toLocaleString("ko-KR")}㎡` : "경계 미확정"} /><Metric label="경계 둘레" value={project.site.perimeterMeters ? `${Math.round(project.site.perimeterMeters)}m` : "경계 미확정"} /></div>
