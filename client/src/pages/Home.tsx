...
export default function Home() {
  ...
  const addCollectedResearch = (notes: LocalProject["researchNotes"]) => {
    ...
    commit({ ...project, researchNotes: [...notes, ...project.researchNotes], overlays: [...overlays, ...project.overlays.filter(existing => !overlays.some(next => next.id === existing.id))] });
  };
  const confirmParcel = (candidate: { pnu?: string; parcelNumber?: string; landCategory?: string; areaSqm?: string }) => {
    const parcelLabel = [candidate.parcelNumber, candidate.landCategory].filter(Boolean).join(" · ");
    commit({ ...project, site: { ...project.site, pnu: candidate.pnu, parcelLabel }, researchNotes: [{ id: newId(), source: "VWorld 연속지적도", title: "사용자 확정 필지", summary: `PNU ${candidate.pnu ?? "미확인"}, 지번 ${candidate.parcelNumber ?? "미확인"}, 지목 ${candidate.landCategory ?? "미확인"}, 면적 ${candidate.areaSqm ? `${candidate.areaSqm}㎡` : "미확인"}. 지도 경계와 실제 지번의 일치 여부는 사용자가 확인했습니다.`, latitude: project.site.latitude, longitude: project.site.longitude, createdAt: new Date().toISOString() }, ...project.researchNotes] });
    toast.success("필지 후보를 현재 프로젝트에 확정했습니다.");
  };
  ...
  <SiteMapPicker clientId={services.naverMapsClientId ?? ""} latitude={project.site.latitude} longitude={project.site.longitude} address={project.site.address} boundary={project.site.boundary} radiusMeters={project.studyRadiusMeters} overlays={project.overlays} onSiteChange={updateSite} onBoundaryChange={updateBoundary} onOpenSettings={openSettings} />
  <ParcelCandidatePanel site={project.site} vworldKey={services.vworldKey} onConfirm={confirmParcel} onOpenSettings={openSettings} />
