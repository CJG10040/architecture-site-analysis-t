...
import { candidateBoundary, parcelCandidateKey, type VworldParcelCandidate } from "@/static/vworld";
...
export default function Home() {
  ...
  const updateSite = (change: Partial<LocalProject["site"]>) => commit({ ...project, site: { ...project.site, ...change } });
  const updateBoundary = (points: LocalProject["site"]["boundary"]) => {
    const metrics = boundaryMetrics(points);
    updateSite({ boundary: points, geoJson: boundaryGeoJson(points) ?? undefined, areaSqm: metrics.areaSqm || undefined, perimeterMeters: metrics.perimeterMeters || undefined });
  ...
  const confirmParcel = (candidate: VworldParcelCandidate) => {
    const parcelLabel = [candidate.parcelNumber, candidate.landCategory].filter(Boolean).join(" · ");
    const parcelPoints = candidateBoundary(candidate);
    ...
    const nextSite = { ...project.site, pnu: candidate.pnu, parcelLabel };
    if (applyParcelGeometry) Object.assign(nextSite, { boundary: parcelPoints, geoJson: boundaryGeoJson(parcelPoints) ?? undefined, areaSqm: parcelMetrics.areaSqm || undefined, perimeterMeters: parcelMetrics.perimeterMeters || undefined });
    commit({ ...project, site: nextSite, researchNotes: [{ id: newId(), source: "VWorld 연속지적도", title: "사용자 확정 필지", summary: `PNU ${candidate.pnu ?? "미확인"}, 지번 ${candidate.parcelNumber ?? "미확인"}, 지목 ${candidate.landCategory ?? "미확인"}, 면적 ${candidate.areaSqm ? `${candidate.areaSqm}㎡` : "미확인"}. 지도 위 연속지적도 Polygon을 사용자가 선택했습니다.${applyParcelGeometry ? " 기존 대지경계가 없어 선택 필지 geometry를 대지경계로 적용했습니다." : " 기존에 그린 대지경계는 유지했습니다."}`, latitude: project.site.latitude, longitude: project.site.longitude, createdAt: new Date().toISOString() }, ...project.researchNotes] });
    toast.success(applyParcelGeometry ? "필지를 확정하고 대지경계를 적용했습니다." : "필지 후보를 현재 프로젝트에 확정했습니다.");
  };
  ...
  useEffect(() => { setParcelCandidates([]); setSelectedParcelKey(""); }, [project.site.latitude, project.site.longitude]);
  const handleParcelSelect = (candidate: VworldParcelCandidate) => setSelectedParcelKey(parcelCandidateKey(candidate));
