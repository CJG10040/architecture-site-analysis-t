...
export function analyzeSiteEvidence(site: SiteRecord, layers: SpatialLayer[]): SiteEvidenceDigest {
  ...
  const parcelDetail = parcels.slice(0, 30).map(parcel => `${parcel.parcelNumber ?? "지번 미상"}(${parcel.landCategory ?? "지목 미상"}, ${parcel.areaSqm ? `${parcel.areaSqm}㎡` : "면적 미상"})`).join(" / ");
  const facts = [`확정 필지 ${parcels.length}개${parcelDetail ? ` · ${parcelDetail}` : ""}`, siteAreaSqm ? `설계 경계 면적 약 ${Math.round(siteAreaSqm).toLocaleString("ko-KR")}㎡ · ${site.areaSqmSource === "cadastral" ? "필지 공부면적 합계" : site.areaSqmSource === "geometry" ? "필지 geometry 계산값" : "출처 미상"}` : "설계 경계 면적 미확인", `주변 건축물 공간객체 ${buildingLayer?.totalFeatureCount.toLocaleString("ko-KR") ?? 0}개 중 지도 보존 ${buildingLayer?.features.length.toLocaleString("ko-KR") ?? 0}개`, `도로 공간객체 ${roadLayer?.totalFeatureCount.toLocaleString("ko-KR") ?? 0}개 중 지도 보존 ${roadLayer?.features.length.toLocaleString("ko-KR") ?? 0}개`];
